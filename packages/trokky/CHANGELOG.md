...
- 7a03e92: New `s3-media` adapter: media on any S3-compatible object store — R2 from a Node process, plus
  MinIO, Backblaze B2, Spaces, Ceph and AWS. Until now the only way to keep media off local disk
  was the R2 _binding_, which exists only inside a Worker, so a Node install had to have a volume.
