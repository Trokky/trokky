...
export interface StudioConfig {
  ...
  // Platform-agnostic asset serving
  getHTML?: (config: any, studioPath: string) => string
  getAsset?: (assetPath: string) => { content: Buffer | Uint8Array, contentType: string } | null
}
...
export class StudioIntegration {
  ...
  private routes: Map<string, StudioRoute> = new Map()
  private logger = createLogger('core', 'StudioIntegration')
  ...
  private addRoute(method: 'GET' | 'POST' | 'PUT' | 'DELETE', path: string, handler: StudioRoute['handler']): void {
    const key = `${method}:${path}`
    this.logger.debug('Adding Studio route', { method, path })
    this.routes.set(key, { method, path, handler })
  }

  public getRoutes(): StudioRoute[] {
    return Array.from(this.routes.values())
  }

  public findRoute(method: string, path: string): StudioRoute | undefined {
    ...
    const matchingRoutes: { route: StudioRoute, specificity: number }[] = []
