import { SecurityValidator, InvalidInputError, createLogger } from '@trokky/core';
export class TrokkyRoutes {
    core;
    config;
    routes = new Map();
    logger = createLogger('routes', 'TrokkyRoutes');
    constructor(config) {
    ...
    getApiRoutes() {
        return Array.from(this.routes.values()).filter(route => !this.isStaticRoute(route.path));
    }
    getStaticRoutes() {
        return Array.from(this.routes.values()).filter(route => this.isStaticRoute(route.path));
    }
    ...
    async getCollectionStats(request) {
        try {
            // SECURITY: Validate authentication before processing
            await this.validateAuthentication(request);
            const { collection } = request.params;
    ...
    async regenerateVariants(request) {
        try {
            await this.validateAuthentication(request);
            const { id } = request.params;
            SecurityValidator.validateDocumentId(id);
    ...
    async serveMediaVariant(request) {
        try {
    ...
    async checkSlugUniqueness(request) {
        try {
            // SECURITY: Validate authentication before processing
            await this.validateAuthentication(request);
    ...
    async validateAdminAccess(request) {
        const auth = this.config.authentication;
        if (!auth?.enabled) {
    ...
    logAdminAccess(session, path, success) {
    ...
    async updateMedia(request) {
        try {
            ...
            const updatedMedia = await this.core.updateMedia(id, metadata);
    ...
    async refreshToken(request) {
        try {
            ...
            const result = await this.core.refreshAuthToken(refreshToken);
            if (!result) {
    ...
    async createToken(request) {
        try {
            ...
            const result = await this.core.createAppToken(tokenData, createdBy);
            if (!result.success) {
     ...
     */
    async getStudioConfig(request) {
        try {
            await this.validateAuthentication(request);
            // Get studio configuration from global config or fallback
            const studioConfig = global.__TROKKY_STUDIO_CONFIG__ || {
    ...
    getCustomStructureFunction() {
        ...
        return global.__TROKKY_STRUCTURE__ || null;
    }
    ...
    async buildDefaultStructure(user, schemas) {
        const items = [];
    ...
    shouldIncludeSchemaInStructure(schema, user) {
    ...
    getSchemaIcon(schema) {
        ...
        if (name.includes('category') || name.includes('tag'))
            return 'tag';
        if (name.includes('media') || name.includes('image'))
            return 'photo';
        if (name.includes('setting') || name.includes('config'))
            return 'cog';
        if (name.includes('menu') || name.includes('navigation'))
            return 'menu';
        return 'document-text';
    }
    getSearchableFields(schema) {
        ...
        if (schema.fields) {
            ...
            return fieldNames.filter(name => searchableFields.some(searchable => name.toLowerCase().includes(searchable)));
        }
        return ['title'];
    }
    /**
     ...
     */
    async tryAutoCreateSingleton(collection, documentId) {
        try {
            ...
            if (customStructure && typeof customStructure === 'function') {
                ...
                isSingleton = singletonPatterns.some(pattern => pattern.collection === collection && pattern.documentId === documentId);
            }
            if (isSingleton) {
     ...
     */
    getDefaultSingletonData(collection, documentId) {
        switch (collection) {
            case 'homePage':
                return {
                    title: 'Home Page',
