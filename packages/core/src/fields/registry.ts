...
export class FieldTypeRegistrationError extends Error {
  constructor(message: string, public fieldType?: string) {
    super(message)
...
export class FieldTypeRegistry {
  ...
  private static metadata = new Map<string, FieldTypeMetadata>()
  
  /**
   * Register a field type in the registry
   */
  static register<T extends FieldType>(
    fieldType: T, 
    options: FieldTypeRegistrationOptions = {}
  ): void {
    const { override = false, migrations = [] } = options
    
    // Check if field type already exists
    if (this.types.has(fieldType.name) && !override) {
      throw new FieldTypeRegistrationError(
        `Field type '${fieldType.name}' is already registered. Use override option to replace.`,
        fieldType.name
      )
    }
  ...
  static search(query: string): FieldType[] {
    ...
    return this.getAll().filter(fieldType => 
      fieldType.name.toLowerCase().includes(lowerQuery) ||
   ...
   */
  private static validateFieldType(fieldType: FieldType): void {
    if (!fieldType.name) {
    ...
    if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(fieldType.name)) {
      throw new FieldTypeRegistrationError(
        'Field type name must start with a letter and contain only letters, numbers, and underscores'
      )
    }
   ...
   */
  private static removeFromAllCategories(name: string): void {
