...
export class SecurityValidator {
    ...
    static validateCollectionName(name) {
        ...
        if (name.length > this.MAX_COLLECTION_NAME_LENGTH) {
            throw new InvalidInputError(`Collection name too long (max ${this.MAX_COLLECTION_NAME_LENGTH} characters)`, 'collection');
        }
        if (!this.COLLECTION_NAME_REGEX.test(name)) {
            throw new InvalidInputError('Collection name must start with a letter and contain only letters, numbers, underscores, and hyphens', 'collection');
        }
    ...
    static validateDocumentId(id) {
        ...
        if (id.length > this.MAX_ID_LENGTH) {
            throw new InvalidInputError(`Document ID too long (max ${this.MAX_ID_LENGTH} characters)`, 'id');
        }
        if (!this.ID_REGEX.test(id)) {
            throw new InvalidInputError('Document ID must contain only letters, numbers, underscores, and hyphens', 'id');
        }
    }
        ...
        if (options.sort !== undefined) {
            if (typeof options.sort === 'string') {
                ...
                if (options.sort.length > this.MAX_SORT_FIELDS) {
                    throw new InvalidInputError(`Too many sort fields (max ${this.MAX_SORT_FIELDS})`, 'sort');
                }
    ...
    static validateSortField(field) {
        ...
        if (!sortPattern.test(field)) {
            throw new InvalidInputError('Invalid sort field format. Use "field" or "field.asc" or "field.desc"', 'sort');
        }
    }
    ...
    static validateUsername(username) {
        ...
        if (!/^[a-zA-Z0-9_-]+$/.test(username)) {
            throw new InvalidInputError('Username must contain only letters, numbers, underscores, and hyphens', 'username');
        }
