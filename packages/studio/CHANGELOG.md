...
- 67aa925: Studio wears the real Trokky mark. The header placeholder was a primary-tinted square with a
...
- 8526c51: Serve Studio from a Worker: `createStudioFetchHandler` in `@trokky/studio/workers`.
...
- 62f862b: A claim screen for a fresh instance.
...
- 1aa1229: Accept legacy string icon values instead of failing validation on them.
...
- 67f0cac: Move the rich text editor to Tiptap 3, the router to react-router 7, and the build to Vite 8. Together these clear the bulk of the repository's open advisories, from 43 to 7.
...
- db04830: Fix reversed typing in portable text, and conditional hooks across the field layer.
