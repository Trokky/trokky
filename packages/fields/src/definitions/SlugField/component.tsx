...
export function SlugFieldComponent(props: SlugFieldComponentProps) {
  ...
  const inputProps = {
    ...
    placeholder: slugDefinition.placeholder || t('types.slug.placeholder'),
        ...
        {value ? (
          <div className="flex items-center gap-2">
            ...
            {slugDefinition.unique && (
              <span className="text-xs text-gray-500 dark:text-gray-400">({t('types.slug.unique')})</span>
            )}
          </div>
        ) : (
          <span className="text-gray-500 dark:text-gray-400 italic text-sm">{t('types.slug.noSlugSet')}</span>
        )}
        {slugDefinition.source && slugDefinition.autoGenerate && (
          <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            {t('types.slug.autoGeneratedFrom', { source: Array.isArray(slugDefinition.source) ? slugDefinition.source.join(', ') : slugDefinition.source })}
          </div>
        )}
      </div>
    );
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
          ...
          {slugDefinition.unique && value && (
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center">
              {isCheckingUniqueness && (
                <div className="animate-spin h-4 w-4 border-2 border-gray-300 dark:border-gray-500 border-t-blue-500 rounded-full" title={t('types.slug.checkingUniqueness')}></div>
              )}
              {!isCheckingUniqueness && uniquenessStatus === 'unique' && (
                <div className="text-green-500 text-sm" title={t('types.slug.slugIsUnique')}>✓</div>
              )}
              {!isCheckingUniqueness && uniquenessStatus === 'taken' && (
                <div className="text-red-500 text-sm" title={t('types.slug.slugExists')}>✗</div>
              )}
              {!isCheckingUniqueness && uniquenessStatus === 'error' && (
                <div className="text-orange-500 text-sm" title={t('types.slug.couldNotCheck')}>⚠</div>
              )}
            </div>
          )}
        </div>
        ...
        {slugDefinition.source && !slugDefinition.readOnly && !isDisabled && (
          <button
            ...
            title={shouldBeReadOnly ? t('types.slug.editSlug') : t('types.slug.regenerate')}
          >
            {shouldBeReadOnly ? t('types.slug.edit') : '↻'}
          </button>
        )}
      </div>
      ...
      {slugDefinition.source && slugDefinition.autoGenerate && (
        <div className="text-xs text-gray-500 dark:text-gray-400">
          {t('types.slug.autoGeneratedFrom', { source: Array.isArray(slugDefinition.source) ? slugDefinition.source.join(', ') : slugDefinition.source })}
          {isManuallyEdited && ` (${t('types.slug.manuallyEdited')})`}
        </div>
      )}
    </div>
  );
